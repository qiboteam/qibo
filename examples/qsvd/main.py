...
def main(nqubits, subsize, nlayers, nshots, RY, method):
    ...
    initial_state = np.random.uniform(-.5,.5,2**nqubits) + 1j*np.random.uniform(-.5,.5,2**nqubits)
    ...
    if subsize <= nqubits/2:
        reduced_dens = np.trace(densmatrix.reshape(2**n,2**m,2**n,2**m), axis1=1, axis2=3)       
    else:
        reduced_dens = np.trace(densmatrix.reshape(2**n,2**m,2**n,2**m), axis1=0, axis2=2)
