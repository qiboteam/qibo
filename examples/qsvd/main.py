...
parser.add_argument("--subsize", default=3,
                    help="Subsize of the bipartition with qubits 0,1,...,subzize-1", type=int)
parser.add_argument("--nlayers", default=5,
                    help="Number of layers of the variational circuit", type=int)
parser.add_argument("--nshots", default=int(1e5),
                    help="Number of shots used when sampling the circuit", type=int)
...
def main(nqubits, subsize, nlayers, nshots, RY, method):
    ...
    initial_state = np.random.uniform(-.5, .5, 2**nqubits) + \
        1j*np.random.uniform(-.5, .5, 2**nqubits)
    ...
    if subsize <= nqubits/2:
        reduced_dens = np.trace(densmatrix.reshape(
            2**n, 2**m, 2**n, 2**m), axis1=1, axis2=3)
    else:
        reduced_dens = np.trace(densmatrix.reshape(
            2**n, 2**m, 2**n, 2**m), axis1=0, axis2=2)

    schmidt = np.linalg.eigvalsh(reduced_dens)
    schmidt = -np.sort(-schmidt)
    ...
    if not RY:  # if Rx,Rz,Rx rotations are employed in the anstaz
        initial_parameters = 2*np.pi * \
            np.random.rand(6*nqubits*nlayers+3*nqubits)

    else:  # if Ry rotations are employed in the anstaz
        initial_parameters = 2*np.pi * \
            np.random.rand(2*nqubits*nlayers+nqubits)
