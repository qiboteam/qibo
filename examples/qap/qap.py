...
def qubo_qap(g: Tuple[np.ndarray, np.ndarray], penalty: float=None):
    ...
    quadratic = {(i, j): q[i, j] for j in range(q.shape[1])
                                 for i in range(q.shape[0]) if i > j}
...
def qubo_qap_feasibility(g: Tuple[np.ndarray, np.ndarray],
                         solution: Dict[Any, int]):
...
def qubo_qap_energy(g: Tuple[np.ndarray, np.ndarray],
    ...
    for i in range(len(state)):
        energy += np.einsum('i,i->',
                            F[state[i,0], state[:,0]],
                            D[state[i,1], state[:,1]])
    return energy


def hamiltonian_qap(g: Tuple[np.ndarray, np.ndarray], penalty: float=None, dense: bool=False):
