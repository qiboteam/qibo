...
class VQRegressor():
    ...
    def dloss(self, features, labels):
        ...
        for feat, label in zip(features, labels):
            ...
            for i in range(self.nparams):
                loss_gradients[i] += (2 * prediction_evaluation * obs_gradients[i]) - (2 * label * obs_gradients[i]) 
    ...
    def AdamDescent(self, learning_rate, m, v, features, labels, iteration,
                    beta_1 = 0.85, beta_2 = 0.99, epsilon = 1e-8):
        """
    ...
    def train_with_psr(self, epochs, learning_rate, batches, J_treshold):
        ...
        for epoch in range(epochs):
            if(epoch !=0 and losses[-1] < J_treshold): 
                print('Desired sensibility is reached, here we stop: ', iteration, ' iteration')
                break
            #shuffle index list
