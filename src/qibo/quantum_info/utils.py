...
def shannon_entropy(probability_array, base:float=2):
    ...
    if any(probability_array) < 0 or any(probability_array) > 1.:
        raise ValueError(
            "All elements of the probability array must be between 0. and 1.."
        )

    if np.sum(probability_array) > 1. + PRECISION_TOL:
        raise ValueError(
            "Probability vector must sum to 1."
        )

    if base == 2:
        log_prob = np.asarray([0. if p == 0. else np.log2(p) for p in probability_array]) \
            if any(probability_array == 0.) else np.log2(probability_array)
    elif base == 10:
        log_prob = np.asarray([0. if p == 0. else np.log10(p) for p in probability_array]) \
            if any(probability_array == 0.) else np.log(probability_array)
    elif base == np.e:
        log_prob = np.asarray([0. if p == 0. else np.log(p) for p in probability_array]) \
            if any(probability_array == 0.) else np.log(probability_array)
    else:
        log_prob = np.asarray([0. if p == 0. else np.log(p) / np.log(base) for p in probability_array]) \
            if any(probability_array == 0.) else np.log(probability_array) / np.log(base)

    return - np.sum(probability_array * log_prob)
