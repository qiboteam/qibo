...
template<typename T>
__global__ void ApplyXSingleControlKernel(T* state, const T* gate, long tk, long tk_reduced, int c) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  const long ck = (long) 1 << c;
  const long i1 = ((long) ((long) i >> c) << (c + 1)) + (i & (ck - 1)) + ck;
...
template<typename T>
__global__ void ApplyXMultiControlKernel(T* state, const T* gate, long tk, long tk_reduced,
                                            int ncontrols, const int * controls, int nqubits) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  auto i1 = i;
...
struct ApplyXFunctor<GPUDevice, T>: BaseOneQubitGateFunctor<GPUDevice, T> {
  ...
  inline void singlecontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                T* state, const T* gate, long tk, long tk_reduced,
                                int c) const override {
    ApplyXSingleControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, c);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long tk, long tk_reduced,
                                 int ncontrols, const int * controls, int nqubits) const override {
    ApplyXMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, ncontrols, controls, nqubits);
  }
};
...
template<typename T>
__global__ void ApplyYSingleControlKernel(T* state, const T* gate, long tk, long tk_reduced, int c) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  const long ck = (long) 1 << c;
  const long i1 = ((long) ((long) i >> c) << (c + 1)) + (i & (ck - 1)) + ck;
...
template<typename T>
__global__ void ApplyYMultiControlKernel(T* state, const T* gate, long tk, long tk_reduced,
                                            int ncontrols, const int * controls, int nqubits) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  auto i1 = i;
...
struct ApplyYFunctor<GPUDevice, T>: BaseOneQubitGateFunctor<GPUDevice, T> {
  ...
  inline void singlecontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                T* state, const T* gate, long tk, long tk_reduced,
                                int c) const override {
    ApplyYSingleControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, c);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long tk, long tk_reduced,
                                 int ncontrols, const int * controls, int nqubits) const override {
    ApplyYMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, ncontrols, controls, nqubits);
  }
};
...
template<typename T>
__global__ void ApplyZSingleControlKernel(T* state, const T* gate, long tk, long tk_reduced, int c) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  const long ck = (long) 1 << c;
  const long i1 = ((long) ((long) i >> c) << (c + 1)) + (i & (ck - 1)) + ck;
...
template<typename T>
__global__ void ApplyZMultiControlKernel(T* state, const T* gate, long tk, long tk_reduced,
                                            int ncontrols, const int * controls, int nqubits) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  auto i1 = i;
...
struct ApplyZFunctor<GPUDevice, T>: BaseOneQubitGateFunctor<GPUDevice, T> {
  ...
  inline void singlecontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                T* state, const T* gate, long tk, long tk_reduced,
                                int c) const override {
    ApplyZSingleControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, c);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long tk, long tk_reduced,
                                 int ncontrols, const int * controls, int nqubits) const override {
    ApplyZMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, ncontrols, controls, nqubits);
  }
};
...
template<typename T>
__global__ void ApplyZPowSingleControlKernel(T* state, const T* gate, long tk, long tk_reduced, int c) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  const long ck = (long) 1 << c;
  const long i1 = ((long) ((long) i >> c) << (c + 1)) + (i & (ck - 1)) + ck;
...
template<typename T>
__global__ void ApplyZPowMultiControlKernel(T* state, const T* gate, long tk, long tk_reduced,
                                            int ncontrols, const int * controls, int nqubits) {
  const auto index = blockIdx.x * blockDim.x + threadIdx.x;
  const auto i = index % tk_reduced + 2 * tk_reduced * int(index / tk_reduced);
  auto i1 = i;
...
struct ApplyZPowFunctor<GPUDevice, T>: BaseOneQubitGateFunctor<GPUDevice, T> {
  inline void nocontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                            T* state, const T* gate, long tk) const override {
    ApplyZPowKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk);
  }

  inline void singlecontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                T* state, const T* gate, long tk, long tk_reduced,
                                int c) const override {
    ApplyZPowSingleControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, c);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long tk, long tk_reduced,
                                 int ncontrols, const int * controls, int nqubits) const override {
    ApplyZPowMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk, tk_reduced, ncontrols, controls, nqubits);
  }
};
...
struct BaseTwoQubitGateFunctor<GPUDevice, T> {
  ...
  virtual void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
  ...
  void operator()(const OpKernelContext* context, const GPUDevice& d, T* state,
    ...
    if (ncontrols == 0) {
      nocontrolwork(d, numBlocks, blockSize, state, gate, targetk1, targetk2, m1, m2);
    }
    else {
      multicontrolwork(d, numBlocks, blockSize, state, gate, tk1, tk2, targetk1, targetk2,
                       m1, m2, ncontrols, tensor_controls, nqubits, t1, t2);
    }
  };
};

template<typename T>
__global__ void ApplyTwoQubitGateKernel(T* state, const T* gate, long tk1, long tk2, int m1, int m2) {
  ...
  state[i2] = cadd(cadd(cmult(gate[8], buffer), cmult(gate[9], buffer1)),
...
template<typename T>
__global__ void ApplyTwoQubitGateMultiControlKernel(T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
                                                    int ncontrols, const int * controls, int nqubits, int t1, int t2) {
  const auto g = blockIdx.x * blockDim.x + threadIdx.x;
  auto i = g;
...
struct ApplyTwoQubitGateFunctor<GPUDevice, T>: BaseTwoQubitGateFunctor<GPUDevice, T> {
  inline void nocontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                            ...
                            int m1, int m2) const override {
    ApplyTwoQubitGateKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk1, tk2, m1, m2);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
                                 int ncontrols, const int* controls, int nqubits, int t1, int t2) const override {
    ApplyTwoQubitGateMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, ctk1, ctk2, tk1, tk2, m1, m2, ncontrols, controls, nqubits, t1, t2);
  }
};


template<typename T>
__global__ void ApplyFsimKernel(T* state, const T* gate, long tk1, long tk2, int m1, int m2) {
  const auto g = blockIdx.x * blockDim.x + threadIdx.x;
  auto i = ((long) ((long) g >> m1) << (m1 + 1)) + (g & (tk1 - 1));
  i = ((long) ((long) i >> m2) << (m2 + 1)) + (i & (tk2 - 1));
...
template<typename T>
__global__ void ApplyFsimMultiControlKernel(T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
                                            int ncontrols, const int * controls, int nqubits, int t1, int t2) {
  const auto g = blockIdx.x * blockDim.x + threadIdx.x;
  auto i = g;
...
struct ApplyFsimFunctor<GPUDevice, T>: BaseTwoQubitGateFunctor<GPUDevice, T> {
  inline void nocontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                            T* state, const T* gate, long tk1, long tk2,
                            int m1, int m2) const override {
    ApplyFsimKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk1, tk2, m1, m2);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
                                 int ncontrols, const int* controls, int nqubits, int t1, int t2) const override {
    ApplyFsimMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, ctk1, ctk2, tk1, tk2, m1, m2, ncontrols, controls, nqubits, t1, t2);
  }
};


template<typename T>
__global__ void ApplySwapKernel(T* state, const T* gate, long tk1, long tk2, int m1, int m2) {
  const auto g = blockIdx.x * blockDim.x + threadIdx.x;
  auto i = ((long) ((long) g >> m1) << (m1 + 1)) + (g & (tk1 - 1));
  i = ((long) ((long) i >> m2) << (m2 + 1)) + (i & (tk2 - 1));
...
template<typename T>
__global__ void ApplySwapMultiControlKernel(T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
                                            int ncontrols, const int * controls, int nqubits, int t1, int t2) {
  const auto g = blockIdx.x * blockDim.x + threadIdx.x;
  auto i = g;
...
struct ApplySwapFunctor<GPUDevice, T>: BaseTwoQubitGateFunctor<GPUDevice, T> {
  inline void nocontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                            T* state, const T* gate, long tk1, long tk2,
                            int m1, int m2) const override {
    ApplySwapKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, tk1, tk2, m1, m2);
  }

  inline void multicontrolwork(const GPUDevice& d, int numBlocks, int blockSize,
                                 T* state, const T* gate, long ctk1, long ctk2, long tk1, long tk2, int m1, int m2,
                                 int ncontrols, const int* controls, int nqubits, int t1, int t2) const override {
    ApplySwapMultiControlKernel<T><<<numBlocks, blockSize, 0, d.stream()>>>(state, gate, ctk1, ctk2, tk1, tk2, m1, m2, ncontrols, controls, nqubits, t1, t2);
  }
};
