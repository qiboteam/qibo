...
def measure_1qb_state(j, circ, noise_model=None):
...
def measure_2qb_state(j, circ, noise_model=None):
...
def sort_counts(matrix):
    ...
    nqubits = int(np.log2(np.shape(matrix)[0]))
    counts_matrix = np.zeros((2**nqubits,2))
...
def GST_measure_1qb(circuit, k, j, nshots, save_data=None):
    ...
    if save_data is not None:
        ...
        if class_qubit_gate is None:
            ...
            if theta is None:
                fname = "%s gate (k=%.2d,j=%.2d) %s_basis.txt" %(class_qubit_gate, k, j, idx_basis[j])
            elif theta is not None:
                fname = "%s(%.12f) gate (k=%.2d,j=%.2d) %s_basis.txt" %(class_qubit_gate, theta, k, j, idx_basis[j])
...
def GST_measure_2qb(circuit, k, j, nshots, save_data=None):
...
def GST_measure_13(circuit, idx_basis_op, k, j, nshots, save_data=None):
    ...
    if save_data is not None:
        ...
        fname = "BasisOp%.2d (k=%.2d,j=%.2d) %s_basis.txt" %(idx_basis_op, k, j, idx_basis[j])
        # If location does not exist, create location.
...
def GST_measure_241(circuit, idx_basis_ops, k, j, nshots, save_data=None):
    ...
    if save_data is not None:
        ...
        fname = "BasisOp%.3d (k=%.2d,j=%.2d) %s_basis.txt" %(idx_basis_op, k, j, idx_basis[j])
        # If location does not exist, create location.
...
def GST_1qb(nshots=int(1e4), noise_model=None, save_data=None, class_qubit_gate=None, theta=None, operator_name=None, backend=None):
    ...
    for k in range(0,4):
        ...
        # 4 measurements
        for j in range(0,4):
            new_circ = measure_1qb_state(j, circ)
...
def GST_2qb(nshots=int(1e4), noise_model=None, save_data=None, class_qubit_gate=None, ctrl_qb=None, targ_qb=None, theta=None, operator_name=None, backend=None):
    ...
    for k in range(0,16):
        ...
        for j in range(0,16):
            new_circ = measure_2qb_state(j, circ)
...
def GST_1qb_basis_operations(nshots=int(1e4), noise_model=None, gjk_1qb=None, save_data=None, backend=None):
    ...
    BasisOps_13 = [gates.I(0).matrix(backend=GlobalBackend()),
    ...
    for idx_basis_op in range(0,13):
        ...
        for k in range(0,4):
            ...
            if  idx_basis_op < 10:
                circ.add(gates.Unitary(BasisOps_13[idx_basis_op], 0, trainable=False, name='%s' %(gatenames[idx_basis_op])))
            ...
            for j in range(0,4**nqubits):            
                new_circ = measure_1qb_state(j, circ)
                expectation_val = GST_measure_13(new_circ, idx_basis_op, k, j, nshots, save_data)
                column_of_js[j,:] = expectation_val

            
            Bjk_tilde_1qb[idx_basis_op, :, k] = column_of_js.reshape(4,)
        idx_basis_op += 1
...
def GST_2qb_basis_operations(nshots=int(1e4), noise_model=None, gjk_2qb=None, save_data=None, backend=None):
    """Runs gate set tomography for the 13 basis operations for a single qubit.
    ...
    for idx_gatenames_1 in range(0,13):
        for idx_gatenames_2 in range(0,13):
            ...
            for k in range(0,16):
                ...
                for j in range(0,4**nqubits):
                    new_circ = measure_2qb_state(j, circ, noise_model=None)
                    expectation_val = GST_measure_241(new_circ, idx_basis_op, k, j, nshots, save_data)
                    column_of_js[j,:] = expectation_val

                Bjk_tilde_2qb[idx_basis_op, :, k] = column_of_js.reshape(16,)
    ...
    for idx_basis_ops_72 in range(169,241):        
        ...
        for k in range(0,16):
            ...
            circ.add(gates.Unitary(BasisOps_241[idx_basis_ops_72], 0,1, trainable=False, name='%s' %(idx_basis_ops_72)))
