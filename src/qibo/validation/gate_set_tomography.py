...
def GST_measure(circuit, k, j, nshots, nqubits=None, class_qubit_gate=None, theta=None, idx_basis_ops=None, save_data=None):
    ...
    if save_data is not None:
        ...
        if class_qubit_gate is None and idx_basis_ops is None:
            ...
            if nqubits == 1:
                fname = 'BasisOp%.2d (k=%.2d,j=%.2d) %s_basis.txt' %(idx_basis_ops, k, j, idx_basis[j])
            elif nqubits == 2:
                fname = 'BasisOp%.3d (k=%.2d,j=%.2d) %s_basis.txt' %(idx_basis_ops, k, j, idx_basis[j])
            folder_name = f'GST_basisops_qibo_%.1dqb' %(nqubits)

        elif class_qubit_gate is not None and idx_basis_ops is not None:
            raise ValueError("Both 'class_qubit_gate' and 'idx_basis_ops' cannot be specified at the same time.")
    ...
    if j==1:
        sorted_matrix = np.hstack((sorted_matrix, expectation_signs[:,0].reshape(-1,1)))
    elif j==2 or j==3 or j==4:
        sorted_matrix = np.hstack((sorted_matrix, expectation_signs[:,1].reshape(-1,1)))
    elif j==5 or j==9 or j==13:
        sorted_matrix = np.hstack((sorted_matrix, expectation_signs[:,2].reshape(-1,1)))
    else:
        sorted_matrix = np.hstack((sorted_matrix, expectation_signs[:,3].reshape(-1,1)))
...
def GST_1qb(nshots=int(1e4), noise_model=None, class_qubit_gate=None, theta=None, operator_name=None, backend=None, save_data=None):
    ...
    for k in range(0,4):
        ...
        for j in range(0,4):
            ...
            expectation_val = GST_measure(new_circ, k, j, nshots, nqubits, class_qubit_gate, theta, idx_basis_ops, save_data)
            matrix_jk_1qb[j, k] = expectation_val
    
    return matrix_jk_1qb 


def GST_2qb(nshots=int(1e4), noise_model=None, class_qubit_gate=None, ctrl_qb=None, targ_qb=None, theta=None, operator_name=None, backend=None, save_data=None,):
