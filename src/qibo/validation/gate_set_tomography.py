...
def reset_register(circuit, invert_register):
    ...
    if invert_register == 'sp_0' or invert_register == 'sp_1':
        ...
        raise NameError(f"{invert_register} not recognized. Input ""sp_0"" to reset qubit 0, ""sp_1"" to reset qubit 1, or ""sp_t"" to reset both qubits.")
...
def GST(nqubits=None, gate=None, nshots=int(1e4), invert_register=None, noise_model=None, backend=None):
    ...
    if gate is not None:
        ...
        if nqubits != qb_gate:
            raise ValueError(f"Mismatched inputs: nqubits given as {nqubits}. {gate} is a {qb_gate}-qubit gate.")
