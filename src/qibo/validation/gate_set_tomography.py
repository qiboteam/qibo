...
def GST(
    ...
    if class_qubit_gate is not None:
        if ctrl_qb is not None and targ_qb is not None:
            ...
            if nqubits != qb_gate:
                raise ValueError(
                    f"Mismatched inputs: nqubits given as {nqubits}. {class_qubit_gate} is a {qb_gate}-qubit gate."
                )
        ...
        if nqubits == 1:
            ...
            for k in range(0, 16):
                ...
                if class_qubit_gate is not None:
                    if theta is None:
                        circ.add(
                            getattr(gates, class_qubit_gate)(targ_qb[0], ctrl_qb[0])
                        )
                    else:
                        circ.add(
                            getattr(gates, class_qubit_gate)(
                                targ_qb[0], ctrl_qb[0], theta
                            )
                        )
...
def GST_basis_operations(
    nqubits=None, nshots=int(1e4), noise_model=None, gjk=None, backend=None
):
    ...
    if nqubits is not None:
        if nqubits == 1:
            ...
            BasisOps_13 = [
                gates.I(0).matrix(backend=backend),
            ...
            interm_ops.append(
                np.kron(xgate, identity) @ CNOT @ np.kron(xgate, identity)
            )
            interm_ops.append(
                np.matrix(
                    np.block([[identity, np.zeros((2, 2))], [np.zeros((2, 2)), sgate]])
                )
            )
            interm_ops.append(
                np.matrix(
                    np.block(
                        [[identity, np.zeros((2, 2))], [np.zeros((2, 2)), hadamard]]
                    )
                )
            )
            interm_ops.append(
                np.kron(ry_piby4, identity) @ CNOT @ np.kron(ry_piby4, identity)
            )
                ...
                for k in range(0, 16):
                    ...
                    circ.add(
                        gates.Unitary(
                            ...
                            1,
                            trainable=False,
                            name="BasisOp%s" % (idx_basis_ops_72),
                        )
                    )
