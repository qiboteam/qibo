...
class LoopMiFGD:
    ...
    def est_Estimation_specNrm(self, stateU):
        ...
        for ii, projector, measurement in zip(
            *[cnt, self.projector_list, self.measurement_list]
        ):
    ...
    def Load_Init(self, Ld):
        ...
        if Ld == "RGD":
            ...
            print(
                " X0 - X0_reconstruct = {}\n".format(
                    np.linalg.norm(X0 - X0_reconstruct)
                )
            )
    ...
    def convergence_check(self):
        ...
        if (
            ...
            xx = np.linalg.norm(
                Col_flip_sign(self.stateU) - Col_flip_sign(self.previous_stateU)
            )
            ...
            XkErrRatio = np.linalg.norm(self.Xk - self.Xk_old) / np.linalg.norm(
                self.Xk_old
            )
            ...
            if self.target_state is not None:
                ...
                if Xk_Err_compare == 1:
                    ...
                    numerator = density_matrix_diff_norm(
                        self.stateU, self.target_state
                    )  # extremely slow
                ...
                fidelity = state_fidelity(
                    self.target_state, self.stateU, validate=False
                )
...
class BasicWorker(BasicParameterInfo, LoopMiFGD):
    ...
    def __init__(self, params_dict, params_MiFGD):
    ...
    def projection_diff(self):
        ...
        for projector, measurement in zip(
            *[self.projector_list, self.measurement_list]
        ):
            # state_diff += self.single_projection_diff(projector, measurement, self.momentum_state)

            # td1 = time.time()
            diffU = self.single_projU_diff(
                projector, measurement, self.momU, self.momUdag
            )
