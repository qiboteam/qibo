...
class RGDOptimizer:
    ...
    def __init__(
        ...
        self.target_dm = (
            self.backend.cast(target_dm) if target_dm is not None else None
        )
    ...
    def _compute_metrics(self) -> Tuple[float, float]:
        ...
        fidelity = float(
            self.backend.np.real(
                self.backend.np.trace(self.Xk @ self.target_dm)
            )
        )
    ...
    def run(
        ...
        if verbose:
            ...
            if self.target_dm is not None:
                print(f"{'Iter':>6} | {'Fro Error':>12} | {'Fidelity':>10} | "
        ...
        if verbose and self.target_dm is not None:
            print(f"{'INIT':>6} | {init_fro_error:>12.6e} | {init_fidelity:>10.6f} | "
                  f"{'---':>12} | {'---':>12}")
        ...
        for k in range(self.max_iterations):
            ...
            if verbose and (k % 1 == 0) and self.target_dm is not None:
                print(f"{stats['iteration']:6d} | {fro_error:12.6e} | "
    ...
    def get_results(self) -> dict:
        ...
        return {
            ...
            'final_fidelity': self.fidelity_history[-1] if self.fidelity_history else None
        }
