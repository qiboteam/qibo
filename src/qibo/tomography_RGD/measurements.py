...
class Measurement:
    ...
    def get_pauli_correlation_measurement(self, beta=None, parity_flavor="effective"):
        ...
        parity_freq2 = {
            k: (-1) ** self.parity(k, parity_flavor) * v for k, v in freq2.items()
        }
    ...
    def get_pauli_basis_measurement(self, beta=None):
        ...
        num_items = len(self.count_dict)
        frequencies = {
            k: (v + beta) / (num_shots + num_items * beta)
...
def measurement_list_calc(label_list, count_dict_list):
    ...
    measurement_object_list = [
        ...
        for (label, count_dict) in zip(*[label_list, count_dict_list])
    ]
...
class MeasurementStore:
    ...
    def calc_measurement_list(cls, label_list, data_dict, method=0):
        ...
        if method == 0:
            ...
            print(
                "       ****   len(label_list) = {},  len(data_dict) = {}\n".format(
                    len(label_list), len(count_dict_list)
                )
            )
            ...
            if num_CPU > len(label_list):
                num_CPU = 3
            print(
                "       ****  use parallel #CPU = {} to calc measurement_list \n".format(
                    num_CPU
                )
            )
            ...
            if Run_parallel == 1:
                L_pt = pool.starmap(
                    measurement_list_calc_wID, zip(ID_list, label_part, count_dict_part)
                )
                pool.close()
                pool.join()
                ...
                ml_dict = {}
                {
                    ml_dict.update(xx) for xx in L_pt
                }  #  if return  {ID, measurement_list}
                ...
                for ii, labels in enumerate(label_part):
                    print(
                        "       **************  {}-th label_part to be parallelly calc   *******".format(
                            ii
                        )
                    )

                    mp_out = pool.apply_async(
                        measurement_list_calc_wID_wrap,
                        ([ii, labels, count_dict_part[ii]],),
                    )
    ...
    @classmethod
    def Save_measurement_by_data_dict(
        cls, path, label_list, data_dict, method=0, Name=None, ToSave=1
    ):
        ...
        tt2 = time.time()
        print(
            "    ******   cls.calc_measurement_list   -->  time = {}   *****\n".format(
                tt2 - tt1
            )
        )
        ...
        if ToSave == 1:
            ...
            tt4 = time.time()
            print(
                "    ******   cls.Save_measurement_list_file   -->  time = {}   ****".format(
                    tt4 - tt3
                )
            )
    ...
    def Load_measurement_list(cls, path, Name=None):
        ...
        print(
            "  --> loading measurement_list DONE from  ml_file = {}\n".format(ml_file)
        )
        return label_list, measurement_list
    ...
    def Save_measurement_list_file(cls, path, label_list, measurement_list, Name=None):
        ...
        print(
            "          ***  measurement_list --> ml_file = {} is saved\n".format(
                ml_file
            )
        )
    ...
    def saveInOne(self, path, Name=None):
        ...
        if Name == None:
            ...
            print(
                "      ***   count_dict file = {} is dumped (i.e. saved) \n".format(
                    Fname
                )
            )
            ...
            print(
                "      ***   label_list  &  count_dict_list  is stored in {} \n".format(
                    label_count_file
                )
            )
    ...
    def load(cls, path, labels=None):
        ...
        # load the store
        measurements = [
            Measurement.load(path, label, num_leading_symbols) for label in labels
        ]
        measurement_dict = {}
