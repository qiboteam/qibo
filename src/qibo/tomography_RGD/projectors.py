...
values_dict = {"I": [1.0, 1.0], "X": [1.0, 1.0], "Y": [-1.0j, 1.0j], "Z": [1.0, -1.0]}
...
def generate_random_label_list(
    size, n, Rm_Id=0, symbols=["I", "X", "Y", "Z"], factor=1.0, factor_step=0.1
):
...
class ProjectorStore:
    ...
    def combine_proj_bulk(cls, proj_path, method_combine=1):
        ...
        proj_lab_files = [
            xx for xx in os.listdir(proj_path) if xx.startswith("labels_")
        ]
        ...
        if method_combine == 0:
            print(
                "   ----------   direct combining Pj_list   (NOT parallel)  --------------- \n"
            )
            ...
            for ii, lab_file in enumerate(proj_lab_files):
                ...
                ID_Pj, label_Pj, Pj_list = cls.Load_Pj_part(
                    ID_label, Fname_lab, Fname_proj
                )
                ...
                td2 = time.time()
                print(
                    "  ---------------  done of updating {}-th Pj from {}".format(
                        ii, Fname_proj
                    )
                )
                print(
            ...
            if bulk_Pj < num_cpus:
                num_cpus = bulk_Pj
            print(
                "      ----------   parallelel #CPU = {}  for combining Pj_list -----\n".format(
                    num_cpus
                )
            )
            ...
            for lab_file in proj_lab_files:
                ...
                res = pool.apply_async(
                    cls.Load_Pj_part_wrap, ([ID_label, Fname_lab, Fname_proj],)
                )
                res_list.append(res)
            ...
            print(
                "  *******  After parallel loading Pj  --> len(res_list) = {}  ******* \n".format(
                    len(res_list)
                )
            )
            ...
            for ii in range(bulk_Pj):
                ...
                print(
                    "    --------  ID = {} is popped from res_list  ---------- ".format(
                        ID_Pj
                    )
                )
        ...
        if method_combine == 1:
            print(
                "      ID order of returning from parallel CPU  = {}\n".format(ID_order)
            )
        print(
            "       ----------     Total # bulk Pj file = {}   ---------------".format(
                bulk_Pj
            )
        )
    ...
    def Load_Pj_part(cls, ID_label, Fname_lab, Fname_proj):
        ...
        print(
            "         Start to load Fname_lab = {}   -->  ID_label = {}".format(
                Fname_lab, ID_label
            )
        )
    ...
    def mpPool_map(self, path, bulksize=6000):
        ...
        if self.size > bulksize:
            ...
            label_part = [
                self.labels[ii * bulksize : (ii + 1) * bulksize] for ii in range(numB)
            ]
        ...
        if method == 0:
            ...
            for ii, labels in enumerate(label_part):
                ...
                NowSize += len(labels)
                print(
                    "  ***  Proj {}-th bulk:  # {} projectors were created".format(
                        ii, NowSize
                    )
                )
                ...
                if int(NowSize / mp_num_max) >= saveP:
                    ...
                    print(
                        "  ***  {}-th labels_part  ->  saved bulk size = {}, NowSize = {}".format(
                            ii, len(labels_collect), NowSize
                        )
                    )
                    print("  #########       restart pool      #########\n")
        ...
        print(
            "     [projectors] num_cpus = {},  saved bulk Pj = {} \n".format(
                num_cpus, saveP
            )
        )
    ...
    def populate(self, path):
        ...
        print(
            "  -- num_cpus = {}, num_cpus2= {}, size = {} -- ".format(
                num_cpus, num_cpus2, self.size
            )
        )
            ...
            for t in range(num_cpus2):
                ...
                if format == "pickle":
                    process = multiprocessing.Process(
                        target=_pickle_saver, args=(label, path)
                    )
                elif format == "hdf5":
                    process = multiprocessing.Process(
                        target=_hdf5_saver, args=(label, path, lock)
                    )
    ...
    def load_labels(cls, path, Nk=None):
        if path.endswith(".hdf5"):
            ...
            try:
                ...
                label_load2 = [
                    label for label in label_load1 if not label.startswith("Pj_list")
                ]
                label_load3 = [
                    label for label in label_load2 if not label.startswith("labels")
                ]
                labels = [
                    label for label in label_load3 if not label.startswith("ALL_")
                ]
                ...
                fragment_paths = [
                    os.path.join(path, fragment) for fragment in os.listdir(path)
                ]
                labels = []
                for fragment_path in fragment_paths:
                    fragment_labels = [
                        fname.split(".")[0] for fname in os.listdir(fragment_path)
                    ]
    ...
    def load(cls, path, labels=None):
        ...
        # load the store
        projectors = [
            Projector.load(path, label, num_leading_symbols) for label in labels
        ]
        projector_dict = {}
