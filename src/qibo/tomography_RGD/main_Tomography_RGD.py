...
def qibochem_measure_expectation(circuit, labels, num_shots):
    ...
    for label in labels:
        ...
        coef_Pauli_shots.append(expectation_from_samples(_circuit, symbolPauli, n_shots=num_shots))
...
if __name__ == "__main__":
    ...
    ############################################################
    num_labels = 5
    n = 3;      labels = [generate_random_label(n) for i in range(num_labels)]
    ...
    coef_Pauli_exact, measurement_list = qibochem_measure_expectation(stateGHZ, labels, num_shots)
