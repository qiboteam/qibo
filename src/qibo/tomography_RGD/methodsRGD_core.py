...
class BasicWorkerRGD(LoopRGD):
    ...
    def Amea(self, proj_list, XX):
        ...
        yMea = [
            np.dot(
                proj_list[ii].matrix.data,
                np.asarray(
                    XX[proj_list[ii].matrix.col, proj_list[ii].matrix.row]
                ).reshape(-1),
            ).real
    ...
    def Hr_tangentWk(self, Alpha, uGv):
        ...
        print(
            "    ------------  DONE svd of D2s with {}-th scaling (#max sc={})".format(
                ii_sc, max_scaling_time
            )
        )
        ...
        print(
            "         error of Hermitian = LA.norm(|uk| - |vk|) = {}".format(
                LA.norm(Col_flip_sign(uk) - Col_flip_sign(vk))
            )
        )
    ...
    def calc_PtG_2_uG_Gv(self):
        ...
        if self.EigV_positive == 1:  #  U = V
            ...
            if Have_Amea == 1:
                ...
                for measurement, proj in zip(
                    self.measurement_list, self.projector_list
                ):
    ...
    def calc_n1_n2(self):
        ...
        if self.Md_tr == 0:
            AptGk = Amea(
                self.projector_list, self.PtGk, self.num_labels, self.coef
            )  # quickest
            ...
            n2 = np.sqrt(
                LA.norm(AptGk) ** 2 + A0**2 * self.w0
            )  # weighted sum in the inner product
...
def density_matrix_norm(state):
    ...
    norm = np.sqrt(
        sum([v**2 for v in [np.linalg.norm(state * item) for item in conj_state]])
    )
    return norm
    ...
    norm = np.sqrt(
        sum(
            [
                v**2
                for v in [
                    np.linalg.norm(xstate * xitem - ystate * yitem)
...
def Compare_2_SVD(u1, s1, v1h, u2, s2, v2h):
    ...
    print(
        "    np.allclose(u1,  u2)  = {}".format(
            np.allclose(Col_flip_sign(u1), Col_flip_sign(u2))
        )
    )

    print(
        "    np.allclose(vkh, v5h) = {}".format(
            np.allclose(Row_flip_sign(v1h), Row_flip_sign(v2h))
        )
    )
    print(
        "    np.allclose(vk,  v5)  = {}".format(
            np.allclose(Col_flip_sign(v1h.T.conj()), Col_flip_sign(v2h.T.conj()))
        )
    )

    print(
        "    np.allclose(xR1,  xR2)  = {}".format(
            np.allclose(u1 @ s1 @ v1h, u2 @ s2 @ v2h)
        )
    )
...
def calc_n1_n2(zm, Ai, uk, vk, ukh, vkh, dd, m):
    ...
    Y2t = (
        ukh @ GkV
    )  ##  \in (r, r) | GkV = xx = reduce(lambda x,y: x+y, zEi) \in (dd, r)
