...
def prepare_strings(qubits, nqubits, batch=False):
    batch = int(batch)
    if nqubits + len(qubits) + batch > len(EINSUM_CHARS):  # pragma: no cover
        raise_error(NotImplementedError, "Not enough einsum characters.")

    inp = list(EINSUM_CHARS[: nqubits + batch])
    out = inp[:]
    trans = list(EINSUM_CHARS[nqubits + batch : nqubits + len(qubits) + batch])
    ...
    out = "".join(out)
    trans = "".join(trans)
    rest = EINSUM_CHARS[nqubits + len(qubits) + batch :]
    return inp, out, trans, rest


def apply_gate_string(qubits, nqubits, batch=False):
    inp, out, trans, _ = prepare_strings(qubits, nqubits, batch)
...
def control_order(gate, nqubits, batch=False):
    loop_start = 0
