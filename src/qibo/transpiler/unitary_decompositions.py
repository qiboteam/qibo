...
def calculate_single_qubit_unitaries(psi, backend=None):
    ...
    if (
        backend.real(backend.calculate_norm_density_matrix(backend.imag(psi_magic)))
        > 1e-6
    ):  # pragma: no cover
    ...
    phase = (
        1j * np.sqrt(2) * backend.sum(backend.multiply(backend.conj(ef_), psi_bar[2]))
    )
...
def calculate_diagonal(unitary, ua, ub, va, vb, backend):
    ...
    v_dagger = backend.transpose(backend.conj(backend.kron(va, vb)), (1, 0))
...
def magic_decomposition(unitary, backend=None):
    ...
    ua, ub = backend.transpose(backend.conj(ua_dagger), (1, 0)), backend.transpose(
        backend.conj(ub_dagger), (1, 0)
    )
