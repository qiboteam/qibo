...
class MPLDrawer:
    ...
    def _plot_quantum_circuit(
        self, gates, inits, labels=[], plot_labels=True, **kwargs
    ):
        """Use Matplotlib to plot a quantum circuit.
    ...
    def _draw_controls(
        self, ax, i, gate, labels, gate_grid, wire_grid, plot_params, measured={}
    ):
    ...
    def _draw_labels(self, ax, labels, inits, gate_grid, wire_grid, plot_params):
        ...
        for i in range(nq):
            ...
            self._text(
                ...
                wire_grid[j],
                self._render_label(labels[i], inits),
                plot_params,
            )
    ...
    def _get_flipped_indices(self, targets, labels):
        return [self._get_flipped_index(t, labels) for t in targets]
    ...
    def plot_qibo_circuit(self, circuit, scale, cluster_gates):
        ...
        if len(circuit.queue) > 0:
            ...
            if cluster_gates:
                ...
                return self._plot_quantum_schedule(
                    gates_cluster, inits, labels, scale=scale
                )
