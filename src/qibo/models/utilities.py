...
class Loss_Utils(object):
    ...
    def cvar(hamiltonian, state, alpha=0.1):
        ...
        """
        counts = Loss_Utils.convert_state_to_count(state)
        probabilities = np.zeros(len(counts))
        values = np.zeros(len(counts))
        for i, (x, p) in enumerate(counts.items()):
            values[i] = Loss_Utils.convert_bit_to_energy(hamiltonian, x)
            probabilities[i] = p
        cvar_ans = Loss_Utils.compute_cvar(probabilities, values, alpha)
        return cvar_ans
    ...
    def gibbs(hamiltonian, state, eta=0.1):
        ...
        for bitstring, count in counts.items():
            obj = Loss_Utils.convert_bit_to_energy(hamiltonian, bitstring)
            avg += np.exp(-eta * obj)
            sum_count += count
