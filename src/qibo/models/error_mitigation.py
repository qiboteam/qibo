...
class ErrorMitigation(ABC):
    """Abstract object for implementing error mitigation methods."""
    @abstractmethod
    def run(self, circuit, observable, init_state=None):
        pass

class ZNE(ErrorMitigation):
    """Zero Noise Extrapolation method for error mitigation.
    ...
    """
    def __init__(self, CNOT_noise_model=None):
        self.CNOT_noise_model = CNOT_noise_model
    ...
    def get_gammas(c, solve=True):
        ...
        Args:
            c (numpy.ndarray): Array containing the different noise levels, note that in the CNOT insertion paradigm this corresponds to the number of CNOT pairs to be inserted. The canonical ZNE noise models are obtained as 2*c + 1.
    ...
    def get_noisy_circuit(circuit, cj):
        ...
        Returns:
            noisy_circuit: The circuit with the inserted CNOT pairs.
        """
    ...
    def run(self, circuit, observable, c, init_state=None):
        """Runs the Zero Noise Extrapolation.

        Args:
            circuit (qibo.models.circuit.Circuit): Input circuit.
            observable (qibo.): Observable to measure. # not sure whether this is just a numpy.ndarray or some qibo object
        ...
        expected_val = []
        for cj in c:
            noisy_circuit = self.get_noisy_circuit(circuit, cj)
            if self.CNOT_noise_model != None:
                noisy_circuit = self.CNOT_noise_model.apply(noisy_circuit)
            rho = noisy_circuit(initial_state = init_state)
            expected_val.append(observable.dot(rho.state()).trace())
        gamma = self.get_gammas(c, solve=False)
