...
class DoubleBracketIteration:
    ...
    def __call__(
        ...
        """We use convention that :math:`H' = U^\\dagger H U` where :math:`U=e^{-sW}` with
        :math: `W=[D,H]` (or depending on `mode` an approximation, see `eval_dbr_unitary`).
        If :math:`s>0` then for :math:`D = \\Delta(H)` the GWW DBR will give a :math:`\\sigma'-decrease,
