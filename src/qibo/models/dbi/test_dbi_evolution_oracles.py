...
input_hamiltonian_evolution_oracle_text_strings = EvolutionOracle(
    h_input, "ZX", mode_evolution_oracle=EvolutionOracleType.text_strings
)

c1 = input_hamiltonian_evolution_oracle_text_strings.circuit(0.1)
if (
    input_hamiltonian_evolution_oracle_text_strings.mode_evolution_oracle
...
input_hamiltonian_evolution_oracle_hamiltonian_simulation = EvolutionOracle(
...
if (
    input_hamiltonian_evolution_oracle_hamiltonian_simulation.mode_evolution_oracle
...
input_hamiltonian_evolution_oracle_numerical = EvolutionOracle(
    h_input, "ZX", mode_evolution_oracle=EvolutionOracleType.numerical
)

c3 = input_hamiltonian_evolution_oracle_numerical.circuit(2)
if (
    input_hamiltonian_evolution_oracle_numerical.mode_evolution_oracle
...
U2 = c2.unitary()
input_hamiltonian_evolution_oracle_hamiltonian_simulation.mode_evolution_oracle = (
    EvolutionOracleType.numerical
)
print(
    np.linalg.norm(
        U2 - input_hamiltonian_evolution_oracle_hamiltonian_simulation.circuit(2)
    )
)
...
if 0:
    ...
    gci = GroupCommutatorIterationWithEvolutionOracles(
        input_hamiltonian_evolution_oracle_hamiltonian_simulation
    )
    d_0 = SymbolicHamiltonian(symbols.Z(0) * symbols.Z(1) + symbols.Z(2), nqubits=3)

    query_list = gci.group_commutator_query_list(
        0.2, d_0, input_hamiltonian_evolution_oracle_hamiltonian_simulation
    )
    gci(0.2, d_0)
    ...
    print(
        np.linalg.norm(
            gci.h.exp(0.3)
            - frame_shifted_input_hamiltonian_evolution_oracle.circuit(0.3).unitary()
        )
    )
