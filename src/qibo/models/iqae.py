...
class IQAE:
    ...
    def __init__(
        ...
        if not 0 < epsilon <= 0.5:
            raise ValueError(f"Epsilon must be in (0, 0.5], but is {epsilon}")

        if not 0 < alpha < 1:
            raise ValueError(f"The confidence level alpha must be in (0, 1), but is {alpha}")
            
        if method not in {"chernoff", "beta"}:
            raise ValueError(f"The confidence interval method must be chernoff or beta, but is {method}")
    ...
    def calc_L_range_CP(self, N_shots,T):
        ...
        alpha=self.alpha
        def a_calc_CP(N_total_plus,N_total_shots,alpha,T):
        ...
        y = [(lambda t: a_calc_CP(int(t*N_shots),N_shots,alpha,T))(t) for t in x[tuple(x_domain)]]
        L_max=np.max(y)/(2*np.pi)
        L_min=np.min(y)/(2*np.pi)
        return L_max,L_min
        #return np.min(y)/(2*np.pi),np.max(y)/(2*np.pi)
    ...
    def execute(self,  backend=None):
        ...
        while theta_dif > 2*eps:
            ...
            if self.method == 'chernoff':
                ...
                a_min_i, a_max_i = proportion_confint(sum_total_samples, N_total_shots, method=self.method, alpha=self.alpha/T) 
