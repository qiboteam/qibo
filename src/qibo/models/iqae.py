...
class IQAE:
    ...
    def calc_L_range_CP(self, N_shots, T):
        ...
        def a_calc_CP(N_total_plus, N_total_shots, alpha, T):
            ...
            a_min, a_max = proportion_confint(
                N_total_plus, N_total_shots, method="beta", alpha=(alpha / T)
            )
            return np.abs(np.arccos(1 - 2 * a_max) - np.arccos(1 - 2 * a_min)) / 2

        y = [
            (lambda t: a_calc_CP(int(t * N_shots), N_shots, alpha, T))(t)
    ...
    def execute(self, backend=None):
        ...
        while theta_dif > 2 * eps:
            ...
            if self.method == "chernoff":
                ...
                a_min_i, a_max_i = proportion_confint(
                    sum_total_samples,
                    N_total_shots,
                    method=self.method,
                    alpha=self.alpha / T,
