...
def plot_density_hist(circuit):
    ...
    # Ticks length
    number_of_states_x = len([bin(i)[2:].zfill(circuit.nqubits) for i in range(state.shape[0])])
    number_of_states_y = len([bin(i)[2:].zfill(circuit.nqubits) for i in range(2 ** circuit.nqubits)])
    ax.set_xticks(0.5 + np.arange( number_of_states_x))
    ax.set_yticks(0.5 + np.arange(number_of_states_y))
    
    # Ticklabels
    ax.set_xticklabels([bin(i)[2:].zfill(circuit.nqubits) for i in range(state.shape[0])])
    ax.set_yticklabels([bin(i)[2:].zfill(circuit.nqubits) for i in range(2 ** circuit.nqubits)])
    ax.set_title('Real')
    ...
    ax1.set_xticklabels([bin(i)[2:].zfill(circuit.nqubits) for i in range(state.shape[0])])
    ax1.set_yticklabels([bin(i)[2:].zfill(circuit.nqubits) for i in range(2 ** circuit.nqubits)])
