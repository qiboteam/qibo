...
def test_qulacs(density_matrix, with_measurements):
    circuit = random_clifford(3, backend=numpy_bkd, density_matrix=density_matrix)
    ...
    numpy_bkd.assert_allclose(
        numpy_res.probabilities(), qulacs_res.probabilities(), atol=1e-8
    )
    if with_measurements:
...
def test_initial_state_error():
    ...
    with pytest.raises(NotImplementedError):
        qulacs_bkd.execute_circuit(circuit, initial_state=initial_state)
