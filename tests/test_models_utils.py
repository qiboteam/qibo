...
def test_fourier_coefficients_raising_errors():
    fourier_coefficients(function, 1, 2, lowpass_filter=True)
    with pytest.raises(ValueError):
        # n_inputs!=len(degree)
        fourier_coefficients(function, n_inputs=1, degree=[2, 2], lowpass_filter=True)
    with pytest.raises(ValueError):
    ...
    fourier_coefficients(function, n_inputs=1, degree=2, lowpass_filter=False)
