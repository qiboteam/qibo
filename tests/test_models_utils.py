...
def test_fourier_coefficients_raising_errors():
    ...
    with pytest.raises(ValueError):
        #n_inputs!=len(degree)
        result =  fourier_coefficients(function, n_inputs=2, degree=2, lowpass_filter=True)
    with pytest.raises(ValueError):
        #n_inputs!=len(filter_threshold)
        result =  fourier_coefficients(function, 1,2, lowpass_filter=True,filter_threshold=[5,6])
    result =  fourier_coefficients(function, n_inputs=1, degree=2, lowpass_filter=True,filter_threshold=5)
