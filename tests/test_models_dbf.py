...
def test_double_bracket_flow_canonical(backend, nqubits):
    ...
    for _ in range(NSTEPS):
        dbf(mode=DoubleBracketFlowMode.canonical, step=np.sqrt(0.001))
...
def test_double_bracket_flow_group_commutator(backend, nqubits):
    ...
    with pytest.raises(ValueError):
        dbf(mode=DoubleBracketFlowMode.group_commutator, step=0.01)

    for _ in range(NSTEPS):
        dbf(mode=DoubleBracketFlowMode.group_commutator, step=0.01, d=d)
