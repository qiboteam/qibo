...
def test_trotter_hamiltonian_three_qubit_term(backend):
    ...
    mm1 = backend.kron(m1, eye)
    mm2 = backend.kron(backend.kron(eye, eye), m2)
    mm3 = backend.kron(backend.kron(eye, m3), backend.kron(eye, eye))
    ...
    final_state = backend.execute_circuit(circuit, backend.copy(initial_state)).state()
    mm1 = backend.to_numpy(mm1)
    mm2 = backend.to_numpy(mm2)
    mm3 = backend.to_numpy(mm3)
    u = [expm(-0.5j * dt * (mm1 + mm3)), expm(-0.5j * dt * mm2)]
    u = backend.cast(u)
    target_state = backend.matmul(u[1], backend.matmul(u[0], initial_state))
    target_state = backend.matmul(u[0], backend.matmul(u[1], target_state))
